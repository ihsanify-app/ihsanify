...
function buildCarbonizeReplacement({
	...
	const pushCarbonizeBody = (bodyIndent) => {
		...
		lines.push(
			`${bodyIndent + commentSyntax.open} impeccable-carbonize-start ${id} ${commentSyntax.close}`,
		);
		lines.push(
		...
		lines.push(
			`${bodyIndent + commentSyntax.open} impeccable-carbonize-end ${id} ${commentSyntax.close}`,
		);
		lines.push(
