...
**Root cause:** Payroll (`buildPayslipPreview` in `payroll.ts`) determined "which groups does this teacher teach, and who's enrolled" by replaying the `GroupTeacher`/`GroupEnrollment` audit logs at one cutoff (`asOf = endOfMonth`), not by reading anything recorded on the sessions themselves. Two failure modes fell out of that: (1) the backdate feature stamps every edit made while viewing the same past month with the *identical* timestamp, and `getCurrentTeacherId`'s `orderBy: { date: "desc" }` has no secondary sort key — so a tied-timestamp lookup can resolve to the wrong "current" teacher, making the roster-diff check think nothing changed and silently drop a real edit; (2) a student's billing eligibility was computed from "currently enrolled," not from whether they actually attended a session that month, so a same-month enrollment-then-attendance sequence could miss the window depending on exact ordering.

**Solution:** Added `Session.teacherId`, captured once at logging time via `getCurrentTeacherId(groupId, sessionDate)` (anchored to that session's own date, not "now"). Rewrote `buildPayslipPreview` to query `Session` directly by `teacherId` + period, and to derive the billed student list from actual `SessionAttendance` rows in those sessions rather than a live-enrollment lookup — payroll no longer depends on replaying the assignment/enrollment audit log at read time at all, and a mid-month teacher swap is now attributed correctly instead of all-or-nothing. Existing sessions backfilled via a one-time script (`prisma/backfillSessionTeacher.ts`) that only guesses when a group has had exactly one teacher ever (no ambiguity); groups with more than one are left for manual review rather than guessed at. The underlying tie-ordering bug in `groupState.ts` itself is still unfixed — a more general fix (an explicit `createdAt` tiebreaker) was built and verified, then deliberately reverted at the user's request to keep this fix minimal. It will resurface if multiple roster edits are made while viewing the same backdated month again.
