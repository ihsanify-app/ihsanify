...
- **Impeccable design workflow adopted** (2026-08-27) — installed the `impeccable` Claude Code skill and ran its full flow for the first time: `init` (wrote `apps/platform/PRODUCT.md`), `document` (wrote `apps/platform/DESIGN.md` + `.impeccable/design.json` — "The Nurturing Garden" north star, "Deep Sage Trust" green as the only brand-carrying color, Stone as the only sanctioned neutral, Baloo 2/Nunito typography, a "One Green Rule"/"One Glow Rule" pair of named constraints), then `critique` on both the landing page (27/40, Acceptable) and the Dashboard (16/40, Poor) using the skill's mandatory dual-sub-agent method (an isolated design-review agent + an isolated detector/evidence agent, synthesized after both finish).
  - **Dashboard fixes applied** (top-3 scope, user-selected): `colorize` — every `gray-*` class in GroupCalendar/MiniCalendar/NearestSessionCard replaced with Stone/Sage, three card borders upgraded to the signature `border-green-100`; `typeset` — each widget's primary label promoted to a real `<h2 className="font-heading">` at Title scale, fixing both the flat-typography finding and the missing-heading-landmark accessibility gap, plus `aria-label`s added to icon-only calendar nav buttons; `harden` — the attendance-save and `/groups`-fetch failure paths now show a visible, retryable error state instead of silently reverting to "looks like nothing to do." Left standing on purpose: the ungoverned per-group `cardColor` hex (a separate P1 not in this round's scope).
  - **Landing page fixes applied** (all 3 recommended items): `harden` on Stats.tsx (loading skeleton instead of a misleading "0+", hides the whole stats bar on a real fetch failure rather than showing false zeros); `harden` on InstagramMarquee.tsx (the six placeholder-card gradients and the header's stray pink icon reskinned to green/stone-only combinations, resolving the detector's `ai-color-palette` finding); Testimonials.tsx initially **removed entirely** rather than inventing replacement quotes for content PRODUCT.md already flagged as likely-fabricated — superseded same-day by the real fix below once the user specified what they actually wanted.
  - One confirmed detector false positive persisted as a disclosed, narrowly-scoped ignore rather than left to keep re-firing: `gray-on-color` on `Navbar.tsx`'s nav-link hover state (`text-stone-600` at rest, `bg-green-50` only on `:hover` alongside `hover:text-green-700` — the flagged pair never actually renders together; the detector does static className co-occurrence matching without modeling Tailwind's `hover:` pairing). Recorded via `hook-admin.mjs ignore-value` with the reasoning attached, per the skill's self-serve suppression rules.
  - Both critique/fix rounds verified with a live re-run of `detect.mjs` after each batch (Dashboard: only the one known, deliberately-deferred finding remains; landing page: zero findings) plus `tsc`/`biome` clean and a live page-load check — not just "the edit compiled."
- **Testimonials rebuilt as a real admin-managed feature** (2026-08-29) — the user's actual fix for the fabricated-testimonials finding: not removal, but a genuine content-management flow so admin can input real ones.
  - **New model**: `Testimonial` (`name`, `message`, `createdAt`) — deliberately minimal, no fields not asked for. Public read (`GET /public/testimonials`, no auth — this is exactly the content it's meant to display publicly) with admin-gated `POST`/`PATCH`/`DELETE` in `apps/api/src/routes/testimonials.ts`, mirroring the same public-read/admin-write split already used for Instagram/stats/branding in `public.ts`.
  - **New Settings tab**: `/settings/landing` (`SettingsTabs` gained a "Landing" entry) — add/edit/delete testimonials as cards, same modal/confirm-delete pattern as every other admin CRUD page in this app.
  - **Landing redesign — a deliberate pastiche, not palette drift**: the user explicitly asked for the cards to look like a WhatsApp chat background, so `Testimonials.tsx` uses a warm wallpaper-toned section background, white message-bubble cards with a small tail, the sender's name in bold green above the message (WhatsApp's own group-chat convention), and a timestamp + blue double-check — the wallpaper tone and the blue check are WhatsApp's own signature details, intentionally outside DESIGN.md's usual green-only accent language because the brief specifically called for them (`craft-floor.md`: "the brief's own words can earn any of them"). The section hides entirely when there are zero testimonials, matching the same "hide rather than show fake/broken content" precedent established by the Stats.tsx fix above, rather than reintroducing placeholder content.
  - Verified end-to-end via the real API, not just typecheck: created a testimonial unauthenticated (401), as a non-admin teacher (403), as admin (201, immediately visible on the public endpoint), edited it, deleted it, and confirmed it disappears from the public endpoint — then cleaned up so the table is genuinely empty, ready for the admin's real input.
  - **`givenAt` added** (same day) — the user noticed the bubble's timestamp was `createdAt` (when the admin typed it into the form), not when the testimony was actually given; batch-entering old real testimonials would've shown today's date on all of them. Added a separate `givenAt` field (defaults to now, editable via a `datetime-local` input in the Settings → Landing form), left `createdAt` as pure system record-keeping. The public list now orders by `givenAt` desc, and the bubble's timestamp follows WhatsApp's own real convention (just the time for today, `"15 Mei, 09:30"`-style otherwise).
  - **Seed data source of truth added** (same day) — `apps/api/prisma/seed-data/testimonials.json` captures the real testimonials as they exist in the admin-managed table, and `prisma/seed.ts` reads it on every `pnpm db:seed` run, creating any entry whose exact `(name, message)` pair doesn't already exist. Same idempotency convention as every other seed in that file (matched by content/name, not an id — editing a testimonial's message via the UI and reseeding creates a second row rather than updating the first, since this is a one-way "known-good baseline," not a live mirror of the table). Verified two ways: ran `pnpm db:seed` against the database with the real testimonial already present and confirmed no duplicate was created; separately, inside a Prisma transaction that was deliberately rolled back afterward, wiped the table to simulate a genuinely fresh install, ran the exact seed logic, confirmed it recreated the real testimonial correctly, then rolled back so no real data was ever at risk.
  - **Multi-line testimonial content — a real data-entry footgun found and fixed**: a testimonial entered with literal `\n` text (copied from the JSON file's escaped-string view rather than typed as real line breaks) rendered as one run-on line in the browser — HTML collapses literal newline *characters*, but a plain `<textarea>` also never interprets the two-character sequence `\` + `n` as anything special, so pasted "\n" stayed literal text instead of becoming a break either way. Fixed two ways: added `whitespace-pre-line` to both the landing bubble and the Settings → Landing card preview (so real newline characters, whichever way they get in, actually render); patched the one live record that had the literal-text bug via the API directly. Going forward, real line breaks in `/settings/landing`'s textarea come from pressing Enter, not from typing `\n` — that escape syntax only means anything inside `testimonials.json`, which gets parsed as JSON.
- **`apps/admin` dev server fixed** (2026-08-29) — two separate bugs surfaced only when actually running `pnpm dev` (all workspaces together), neither caught by typecheck:
  - `vite.config.ts` referenced `rootPackageJson.version` without ever importing it — copy-pasted from `apps/platform`'s config but missing the `createRequire`/`rootPackageJson` setup that config actually has. `ReferenceError` on every admin dev-server boot. Fixed by copying that setup over.
  - Once admin could boot, it collided with `apps/platform` on port `42069` — both configs load TanStack Devtools, which binds a fixed event-bus port by default, and two apps running concurrently under one `pnpm dev` can't share it. Gave admin its own port (`eventBusConfig: { port: 42070 }`), platform keeps the default.
  - Both verified by actually starting admin and platform together (the exact failing scenario), not just isolated boots.
- **Local login broken by a stale `VITE_API_URL`** (2026-08-29) — `.env` had `VITE_API_URL=http://api.ilminnaafi.com` left over from prep for a real deployment, silently pointing the *local* dev frontend at the *remote* API. That remote server's CORS config doesn't allow `http://localhost:3000` as an origin, so every local login attempt was blocked by the browser with no useful on-screen error (curl worked fine throughout, since curl doesn't enforce CORS — the bug was invisible to anything that wasn't an actual browser). Fixed by commenting out `VITE_API_URL` for local dev (uncomment for a production build). Separately, while diagnosing, found the *remote* server's CORS also rejects `https://ilminnaafi.com` (only allows the plain-`http` form) — a real bug on that deployment, left unfixed since it lives in a different server's environment, not this repo.
- **Web Analytics placeholder menu added, undocumented until now** — `/analytics` (admin-only, `LineChart` icon) shows a plain "Coming Soon" card. Added when the user asked to reserve the menu slot for a future web-analytics feature (internal usage tracking vs. a third-party embed — genuinely different scopes, not yet decided) without committing to either yet.
- **`.gitignore` updated for the Impeccable skill's generated files** — `PRODUCT.md`, `DESIGN.md`, and `.impeccable/design.json`/`config.json` are kept trackable (durable project docs + the persisted false-positive ignore for `gray-on-color` on `Navbar.tsx`); `.impeccable/hook.cache.json` (pure local scan cache) and `.impeccable/critique/` (point-in-time, regenerable review snapshots) are ignored. The skill package itself (`.claude/skills/impeccable/`) was deliberately left as the user's own call — trivial to reinstall (`npx impeccable install`) either way, so neither committing nor ignoring it was assumed.
- **Browser tab title corrected** — was the literal string `"Ihsanify"` (the LMS/tech brand) in `__root.tsx`; changed to `"Madrasatul 'Ilmin Naafi'"` (the actual school, per the brand distinction PRODUCT.md already records — visitors recognize the school, not the platform it runs on).
- **Presence tracking + Log Session date fix (2026-09-03)** — `User.lastActiveAt`, throttled to one write per 60s inside `requireAuth`; `/users` shows a green dot + "Online"/"Active Xm ago"/"Never active" (2-minute online threshold). `Log Session`'s date field now defaults to today via a local-timezone helper instead of blank (`toISOString()` was silently UTC-shifting it).
- **Group cards redesigned (2026-09-03)** — `/groups` cards show an overlapping avatar-stack + real name list instead of a bare student count, and a row of "This Month's Sessions" circles (checkmark + day-of-month) per session logged that month, backed by a new `currentMonthSessionDays` field on `GET /groups`.
- **`mockAuth.ts` → `auth.ts` rename** — `mockUser` renamed `authUser` across all 14 call sites; naming leftover from early mock-data-first development, no behavior change.
- **Payroll made session-driven instead of audit-log-snapshot-driven (2026-09-12/14)** — the real fix behind a reported bug ("Aysar's Class missing from Sabrina's August payslip"). Investigation found the original exclusion was *correct* per the historical-replay design (her `GroupTeacher` assignment was genuinely dated after August), but surfaced a real, separate bug while trying to correct the backdated data through the UI: the backdate feature (`c1f0836`, see that commit) stamps every edit made while viewing the same past month with an *identical* timestamp, and `getCurrentTeacherId`'s `orderBy: { date: "desc" }` has no secondary sort key — so a tied-timestamp diff-check can silently resolve to the wrong "current" state and drop an edit with no error. Root-caused, then fixed properly rather than patched around:
  - New `Session.teacherId` (nullable, backfilled) — captured at logging time via `getCurrentTeacherId(groupId, sessionDate)` (anchored to the session's own date, not "now"), so payroll no longer needs to replay the assignment log at read time at all.
  - `buildPayslipPreview` (`payroll.ts`) rewritten to query `Session` directly by `teacherId` + period, deriving the billed student list from actual per-session `SessionAttendance` rather than `getCurrentStudentIds` — this incidentally fixes a second, independently-reported bug (a student added to "Thalibaat Bhs Arab" mid-month not showing on the payslip) and correctly attributes a mid-month teacher swap instead of all-or-nothing, which the old design never handled at all.
  - One-time backfill script (`prisma/backfillSessionTeacher.ts`, `pnpm db:backfill-session-teacher`) — resolves each existing session's teacher from the audit log at that session's own date, falling back to "the group's sole-ever teacher" only when a session predates any recorded assignment (a seed-data artifact, not a real gap) and *never* guessing when a group has had more than one teacher — those are left null and reported for manual review instead.
  - A more general fix (an explicit `createdAt` tiebreaker on the audit-log tables, closing the underlying ordering bug for *all* future backdated edits, not just this one incident) was built, verified, then deliberately reverted at the user's call to keep this incident's fix minimal — **the tie-ordering bug itself is still live in `groupState.ts`** and will resurface if multiple roster/assignment edits are made while viewing the same backdated month again. Revisit if it does.
- **Reset Password redesigned (2026-09-09)** — admin now types the new password directly (min. 6 chars, validated server-side) instead of the system generating a random one to relay; added a show/hide eye toggle. `generateTemporaryPassword` removed.
- **Payroll pricing hidden from teachers (2026-09-09)** — a teacher's own payslip view/PDF/list never included per-student `price` (what a family is billed) to begin with for the PDF, but the JSON API and detail page did leak it. `GET /payroll/payslips/:id` now omits `price` server-side (not just hidden client-side) unless the caller is `ADMIN`; the Eye icon (detail-page link) on the payslip list is admin-only. To give teachers back a *legitimate* view of what they're being paid for, `GET /payroll/payslips` now also returns each payslip's per-line `students: {studentName, groupType, subjectName}[]` (no price) — replacing the bare `lineCount` number on the list page.
- **Settings → User gained a name filter** — client-side substring filter over the already-loaded user list, with a distinct "no matches" vs. "no users at all" empty state.
- **Teacher public landing profiles (2026-09-14)** — new `Teacher.nickname`/`publicTitle`/`publicBio` (free text, deliberately distinct from the existing formal `title` enum used in report PDFs), admin-authored via a new editor in Settings → User's Edit modal. A teacher appears on the landing page's new "Pengajar Kami" section only once `nickname` and `publicBio` are both set — that's the opt-in, no separate visibility flag. Subjects shown per teacher are derived from their *currently* assigned groups (`getCurrentGroupIdsForTeacher`), not the `TeacherSubject` table — that table turned out to be wired into `seed.ts` only and nowhere in the live app, so it would have been stale/unmaintained data. Same change also added the missing avatar-upload control to the Edit User modal (previously creation-only).
- **Subject content moved from hardcoded to admin-managed (2026-09-14)** — `Subject.description`/`iconUrl`/`videoUrl` added; `ProgramsFacilities.tsx`'s landing cards now fetch from `/public/subjects` instead of a hardcoded 5-entry array (which had already drifted — a 6th real subject, "Tahsin & Tahfizh," wasn't in it). Icon upload reuses the same base64-data-URL + 300KB-cap validation as user avatars, now extracted into a shared `utils/imageValidation.ts` (server) / `lib/imageUpload.ts` (client) instead of duplicated. `videoUrl` accepts any YouTube URL shape (parsed client-side, `lib/youtube.ts`) and renders as a "Watch" button opening a modal embed (`youtube-nocookie.com`) — chosen over replacing the icon or a separate video-gallery section specifically to keep the fast-scan card grid uncluttered while still being contextual to the specific subject a visitor is looking at. Real per-subject icon assets (provided by the user) also wired into `seed.ts` for fresh installs, resized 2000×2000 → 256×256 first to stay well under the same 300KB cap.
- **Alma Mater logos (2026-09-14)** — new `AlmaMater` model (`name`, `logoUrl`), same admin-write/public-read split as `Testimonial`. Managed at Settings → Landing; shown as a logo strip ("Pengajar Kami adalah Alumni Dari") directly below the Teachers section on the landing page, reinforcing the same credibility claim.
- **"Remember me" on login (2026-09-14)** — previously every login persisted via `localStorage` with a fixed 7-day token and there was no session-only option at all. Checking the box (default, preserves prior behavior) still stores in `localStorage`; unchecking stores in `sessionStorage` instead (cleared when the tab closes), with the unused storage explicitly cleared on write so a later opposite-choice login can't leave a stale duplicate. Pure client-side storage-location choice — no backend or token-expiry change.
- **Indonesian code identifiers renamed to English (2026-09-14)** — code-hygiene only, no visible content changed: `ProgramFasilitas`→`ProgramsFacilities`, `MengapaKami`→`WhyUs`, `NiatTujuan`→`VisionMission`, `TentangKami`→`AboutUs`, `Pengajar`→`Teachers`, `Registrasi`→`Registration`, `RegistrasiFormModal`→`RegistrationFormModal` (component/file names + section `id`s + `Navbar.tsx`'s `NAV_ITEMS` ids), verified via a real Playwright run through every nav link post-rename. Displayed Indonesian text (headings, nav labels, body copy) was deliberately left untouched — real content for Indonesian-speaking visitors, not a code identifier.
...
## Future Work — Localization (Indonesian / English / Arabic)
_Added 2026-09-14 — captured per explicit request ("write in the plan for now"). Not scoped or started; nothing below is a commitment to a specific approach yet, just the shape of the problem as it stands today._

**Goal:** a language switcher on the public landing page letting a visitor choose Indonesian, English, or Arabic.

**Why this is a bigger lift than a typical "add i18n" task, specifically for this codebase:**
1. **The landing page's real content isn't in the code — it's in the database, admin-authored.** Testimonials, subject names/descriptions, teacher `publicBio`/`publicTitle`/nickname, alma mater names, `ReportSettings` org name/footer text, the Instagram-marquee captions — all of it was deliberately built as admin-editable content (Settings → Landing / Subject / User), not hardcoded strings a typical i18n library (`react-i18next`, `next-intl`, etc.) can extract and translate at build time. Those tools solve "translate this UI chrome" (buttons, nav labels, section headings) well; they don't solve "this paragraph a real admin typed into a textarea needs to exist in three languages." That second problem needs either (a) three separate content fields per translatable model, (b) a generic `Translation` table keyed by `(model, id, field, locale)`, or (c) a translation API at render time (cost + quality risk, and Islamic/Qur'anic terminology is exactly the kind of content generic machine translation tends to mishandle) — a real data-model decision, not a library choice.
2. **Arabic means RTL, not just a third string set.** The landing page's layout (flex/grid direction, icon placement, the `ArabicLetterIcon` hack already in `ProgramsFacilities.tsx`, text alignment throughout) is currently built LTR-only. A genuine Arabic mode needs `dir="rtl"` handling and a real layout pass, not just swapped-in strings — mirrored spacing/icons, and Tailwind's logical properties (`ps-`/`pe-` instead of `pl-`/`pr-`) where the current code uses physical ones.
3. **Scope boundary still needs deciding**: is this the *public landing page only* (the explicit ask), or does it eventually extend to the authenticated app (`/dashboard`, Settings, etc.)? The landing page is far smaller and Indonesian-authored-content-heavy; the app is larger but mostly UI chrome + names resolved at render time (which *are* i18n-library-friendly). Worth deciding explicitly before starting, since it changes which approach (library vs. DB schema work) is worth the investment.

**Not yet decided, flagged for when this is actually picked up:** UI-chrome i18n library choice, translated-content data model, whether machine translation is acceptable for admin content or everything needs a human translator, RTL scope, and whether locale is a visitor-chosen toggle only (no persistence) or remembered (cookie/localStorage, same pattern as the "Remember me" auth choice).
