...
- **`InstagramMarquee` component + weekly sync job** (2026-08-25) — an infinite left-to-right auto-scrolling row of the account's latest posts (`@ilmin_naafi`), shown right below the Hero.
  - Frontend (`InstagramMarquee.tsx`): pure-CSS marquee (`@keyframes marquee-right` in `styles.css`, same pattern as `fade-slide-up`) — content duplicated exactly 2x at `width: max-content` so `translateX(-50%)` always shifts by exactly one set's width regardless of item count/width; pauses on hover. Edge fade + `border-x-2` frame added so cards don't look abruptly clipped at the viewport edge. Falls back to 6 gradient placeholder cards whenever there's no synced data yet (`posts.length === 0`) — always true today, since no access token is configured.
  - Backend: new `InstagramPost` (synced posts, upserted by Instagram's own media id so re-syncing never duplicates) and `InstagramSettings` (singleton holding the long-lived access token + `lastSyncedAt`, same singleton-row pattern as `ReportSettings`/`InvoiceSettings`) models. `services/instagramSync.ts` calls Instagram's Graph API (`GET /me/media`, 10 latest, fields include `media_url`/`thumbnail_url`/`permalink`), refreshes the access token proactively every ~50 days (well inside its 60-day expiry), and is scheduled via `node-cron` (new dependency) — weekly on Sundays 3am, plus an immediate catch-up run on server boot if the last sync is missing or over a week old. `GET /public/instagram-posts` serves the synced rows, unauthenticated (image + link only, no PII).
  - **Blocked on the user, not code**: needs a Meta Developer app + Instagram API authorization (only doable by whoever controls the `ilmin_naafi` account/login) to produce `INSTAGRAM_ACCESS_TOKEN`. Until that env var is set, the sync silently no-ops (logs a warning) and the marquee shows placeholders — this is intentional graceful degradation, not a bug.
  - **Trade-offs & hidden costs, called out explicitly per standing instruction ([[feedback_explain_server_tradeoffs]])**:
    - Adds an always-on in-process background timer (`node-cron`) — fine on this long-running Node server, but won't work as-is if this API is ever moved to a serverless/edge runtime (would need an external cron hitting an endpoint instead).
    - New third-party dependency: Meta's Instagram API. Subject to Meta's rate limits (a trivial load at "10 items, once a week" but worth knowing the ceiling exists), their ToS, and their own history of renaming/reshaping this exact API (Basic Display API → "Instagram API with Instagram Login") — a future Meta change could silently break the sync and require a code update.
    - **Unverified risk, flagged rather than silently assumed fine**: Instagram Graph API's `media_url`/`thumbnail_url` are commonly *temporary, signed URLs*, not permanent links — if they expire faster than the 7-day sync interval (plausible; not confirmed either way against this specific account yet), the marquee could show broken images for days between syncs. Worth monitoring after the first real sync; the fix if it happens is either syncing more frequently or (more robust, more work) downloading and re-hosting each image ourselves instead of storing Instagram's URL — which would add real storage/bandwidth cost (currently zero, since only URLs are stored, not image bytes).
    - No alerting on sync failure — a failed refresh or API error is only `console.error`'d today; if the token silently expires (refresh call fails repeatedly, or the app is deauthorized on Meta's side), the marquee quietly reverts to placeholders with no notification to anyone.
